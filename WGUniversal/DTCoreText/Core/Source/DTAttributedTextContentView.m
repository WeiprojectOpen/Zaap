...
- (CGSize)intrinsicContentSizeInternal
{
	if (!self.layoutFrame) // creates new layout frame if possible
	{
		return CGSizeMake(-1, -1);  // UIViewNoIntrinsicMetric as of iOS 6
...
{
	CGSize neededSize = [self intrinsicContentSizeInternal]; // creates layout frame if necessary
...
- (void)setFrame:(CGRect)frame
{
	...
	if (oldFrame.size.height < frame.size.height || oldFrame.size.width < frame.size.width)
	{
