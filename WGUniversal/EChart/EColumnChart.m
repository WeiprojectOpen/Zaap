    ...
    if (_dataSource != dataSource)
    {
        ...
        
        {
            ...
            if (_showHorizontalLabelsWithInteger)
            {
                ...
                float heightGap = (self.frame.size.height - _baseOffset.height) / (float)horizontalLabelsCount;
                _fullValueOfTheGraph = valueGap * horizontalLabelsCount;
                for (int i = 0; i <= horizontalLabelsCount; i ++)
                {
                    UIView *horizontalLine = [[UIView alloc] initWithFrame:CGRectMake(_baseOffset.width, heightGap * i, self.frame.size.width - _baseOffset.width, HORIZONTAL_LINE_HEIGHT)];
                    horizontalLine.backgroundColor = ELightGrey;
                    [self addSubview:horizontalLine];
                    
                    EColumnChartLabel *eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(0, -heightGap / 2.0 + heightGap * i, _baseOffset.width, heightGap)];
                    [eColumnChartLabel setTextAlignment:NSTextAlignmentCenter];
                    eColumnChartLabel.text = [[NSString stringWithFormat:@"%ld ", (long)valueGap * (horizontalLabelsCount - i)] stringByAppendingString:[_dataSource highestValueEColumnChart:self].unit];;
            ...
            else
            {
                ...
                float heightGap = (self.frame.size.height-_baseOffset.height) / 10.0;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _bottomLineHeight = BOTTOM_LINE_HEIGHT;
...
- (void)reloadData
{
    ...
    float widthOfTheColumnShouldBe = (self.frame.size.width - _baseOffset.width) / (float)(totalColumnsRequired + (totalColumnsRequired + 1) * 0.5);
    ...
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        ...
        if (nil == eColumn)
        {
            eColumn = [[EColumn alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 1.5)+_baseOffset.width, 0, widthOfTheColumnShouldBe, self.frame.size.height-_baseOffset.height)];
...
//            }
        ...
        {
            eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 1.5)+_baseOffset.width, self.frame.size.height-_baseOffset.height, widthOfTheColumnShouldBe, 20)];
            [eColumnChartLabel setTextAlignment:NSTextAlignmentCenter];
    ...
    {
        UIView *bottomLine = [[UIView alloc] initWithFrame:CGRectMake(_baseOffset.width, self.frame.size.height-_baseOffset.height, 0, _bottomLineHeight)];
        bottomLine.backgroundColor = _bottomLineColor == nil ? [UIColor clearColor] : _bottomLineColor;
        //bottomLine.layer.cornerRadius = 2.0;
        [self addSubview:bottomLine];
        [bottomLine setFrame:CGRectMake(_baseOffset.width, self.frame.size.height-_baseOffset.height, self.frame.size.width-_baseOffset.width, _bottomLineHeight)];
