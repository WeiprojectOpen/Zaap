...
#ifndef XMPP_LOGGING_REMOTE
#if DEBUG
#define XMPP_LOGGING_REMOTE 1
#else
#define XMPP_LOGGING_REMOTE 0
#endif
#endif
...
#define XMPP_LOG_OBJC_MAYBE(async, lvl, flg, ctx, frmt, ...) \
    do{ if(XMPP_LOGGING_ENABLED) {if(XMPP_LOGGING_REMOTE && (lvl & flg)) LogMessageCompat(frmt, ##__VA_ARGS__); LOG_MAYBE(async, lvl, flg, ctx, sel_getName(_cmd), frmt, ##__VA_ARGS__);} } while(0)
